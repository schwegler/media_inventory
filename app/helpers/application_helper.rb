...
module ApplicationHelper
  ...
  def render_stars(rating, show_number: false)
    ...
    label = "Rated #{num} out of 5 stars"

    stars_span = content_tag(:span, stars_text, class: 'stars-display', role: 'img', aria: { label: label }, title: label)

    if show_number
      safe_join([stars_span, content_tag(:span, num.to_s, class: 'stars-number')], ' ')
    else
      stars_span
    end
  end
