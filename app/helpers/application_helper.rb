...
module ApplicationHelper
  ...
  def fetch_matching_items(item)
    ...
    # Optimize to eager load :likes, user avatars, and nested comments to prevent N+1 queries in community review lists
    associations = [:likes, { user: { avatar_attachment: :blob }, comments: [:user, :likes, { replies: %i[user likes] }] }]
    if logged_in?
      query.where('is_public = ? OR user_id = ?', true, current_user.id).includes(associations)
    else
      query.where(is_public: true).includes(associations)
    end
  end
end
