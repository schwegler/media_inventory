...
class UsersController < ApplicationController
  ...
  def index
    # ⚡ Bolt: Eager load avatars to prevent N+1 queries in the user directory
  ...
  def following
    @title = 'Following'
    @user  = User.find(params[:id])
    # ⚡ Bolt: Eager load avatars to prevent N+1 queries in the following list
  ...
  def followers
    @title = 'Followers'
    @user  = User.find(params[:id])
    # ⚡ Bolt: Eager load avatars to prevent N+1 queries in the followers list
