...
class UsersController < ApplicationController
  ...
  def index
    # Use with_attached_avatar scope to eliminate N+1 queries when rendering user lists
  ...
  def following
    @title = 'Following'
    @user  = User.find(params[:id])
    # Eager load avatars to prevent N+1 queries in the following list
  ...
  def followers
    @title = 'Followers'
    @user  = User.find(params[:id])
    # Eager load avatars to prevent N+1 queries in the followers list
