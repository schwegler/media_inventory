...
class UsersController < ApplicationController
  ...
  def index
    # Eager load avatars to prevent N+1 queries when listing users
  ...
  def following
    @title = 'Following'
    @user  = User.find(params[:id])
    # Eager load avatars to prevent N+1 queries when listing following
  ...
  def followers
    @title = 'Followers'
    @user  = User.find(params[:id])
    # Eager load avatars to prevent N+1 queries when listing followers
