...
class UsersController < ApplicationController
  ...
  def index
    # Preload user avatars to prevent N+1 queries in directory lists.
  ...
  def following
    @title = 'Following'
    @user  = User.find(params[:id])
    # Preload user avatars to prevent N+1 queries in following list.
  ...
  def followers
    @title = 'Followers'
    @user  = User.find(params[:id])
    # Preload user avatars to prevent N+1 queries in followers list.
