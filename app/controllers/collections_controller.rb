...
class CollectionsController < ApplicationController
  ...
  def fetch_collection_scope(item_type, table_name)
    ...
    # Join corresponding media table for database-level title search filtering using Arel table matching
    join_clause = sanitize_join_sql(table_name)
    arel_table = Arel::Table.new(table_name)
    scope.joins(join_clause).where(arel_table[:title].matches("%#{@query}%"))
  end
