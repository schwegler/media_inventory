...
class User < ApplicationRecord
  ...
  def liked?(likeable)
    # Optimize to prevent O(N) database N+1 queries when checking current_user.liked?(item)
    # across multiple list items by caching all liked items as an in-memory Set.
