...
## 2026-08-09 - N+1 EXISTS Bottleneck in Loops
**Learning:** Checking `likes.exists?(...)` inside views during rendering (e.g., for like buttons on grids/activity feeds) bypasses eager preloading and issues separate database `EXISTS` queries for each element, leading to $O(N)$ database hits.
**Action:** Pluck and cache liked item keys in an in-memory `Set` on the first call to `User#liked?` for $O(1)$ lookups, and clear/invalidate this cache using callbacks (`after_commit`) or controller triggers when likes are modified.
