...
## 2026-08-07 - [A11y Feedback for Async Clipboard Copy Buttons]
**Learning:** When using asynchronous clipboard copy buttons with visual icon swaps, screen reader users are left unaware of successful operations unless ARIA attributes are dynamically updated. Merely changing the icon leaves the state change silent.
**Action:** Dynamically update the copy button's `aria-label` to "Copied!" (or specific context feedback) on success, and restore the original label after the visual feedback window (e.g., 2 seconds) has elapsed.
