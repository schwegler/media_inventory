...
## 2026-06-21 - [Restoring Focus on Modal Dismissal]
**Learning:** Turbo-rendered modals that clear DOM elements leave keyboard focus stranded on `document.body` when closed. Capturing `document.activeElement` during the Stimulus `connect()` phase guarantees focus is cleanly restored to the triggering button upon dismissal.
**Action:** Store `this.previouslyFocusedElement = document.activeElement` in modal controller `connect()` and call `.focus()` on it during `close()`.
