...
RSpec.describe "Admin::EditSuggestions", type: :request do
  let(:admin) { User.create!(name: "Admin User", email: "admin@example.com", password: "password", username: "admin", admin: true) }
  ...
  let(:edit_suggestion) { EditSuggestion.create!(suggestable: movie, user: user, proposed_changes: { "title" => "Approved Title" }) }
